...
public class CameraSource {
    ...
    private static List<SizePair> generateValidPreviewSizeList(Camera camera) {
        ...
        if (validPreviewSizes.size() == 0) {
            Timber.tag(TAG).w("No preview sizes have a corresponding same-aspect-ratio picture size");
            for (Camera.Size previewSize : supportedPreviewSizes) {
    ...
    private void setRotation(Camera camera, Camera.Parameters parameters, int cameraId) {
        ...
        switch (rotation) {
            ...
            case Surface.ROTATION_270:
                ...
                break;
            default:
                Timber.tag(TAG).e("Bad rotation value: %s", rotation);
        }
    ...
    private class FrameProcessingRunnable implements Runnable {
        ...
        public void run() {
            ...
            while (true) {
                synchronized (lock) {
                    while (active && (pendingFrameData == null)) {
                        try {
                            ...
                            Timber.tag(TAG).d(e, "Frame processing loop terminated.");
                            return;
                        }
                    }
                ...
                try {
                    synchronized (processorLock) {
                        Timber.tag(TAG).d("Process an image");
